...
public class Student : Enemy
{
    ...
    public override void LoadContent()
    {
        ...
        TextureAtlas enemyRunningAtlas = TextureAtlas.FromFile(Core.Content, "Animated_Sprites/Student/Run_Student-definition.xml");
        TextureAtlas enemyStandingAtlas = TextureAtlas.FromFile(Core.Content, "Animated_Sprites/Student/Idle_Student-definition.xml");
    ...
    public override void TriggerCollision(Collider collider)
    {
        ...
        if ((collider.GameObject is Player.Player) && (Math.Abs(collider.PhysicsComponent.Velocity.X) > 100))
        {
            collider.PhysicsComponent.Forces.Add(new Vector2(-20*collider.PhysicsComponent.Velocity.X, 0));
        }
    }
}